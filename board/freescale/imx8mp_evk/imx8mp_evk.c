...
#if defined(CONFIG_HMS_TVMB) || defined(CONFIG_HMS_RP103)
#define FEC_RST_PAD IMX_GPIO_NR(1, 7)
#else
...
static void boot_mode_switch(void)
{
    ...
    switch(bm)
    {
        ...
        case BOOT_SD:
            printf("BOOT_SD:\n");
            run_command("mmc dev 1", 0);
            env_set("mmcautodetect", "no");
            env_set("mmcroot", "/dev/mmcblk1p2 rootwait rw");
            env_set("mmcdev", "1");
            if(get_recovery_condition(bm))
            {
                printf("load sdcard p3\n");
                env_set("mmcdev", "1");
                env_set("mmcroot", "/dev/mmcblk1p3 rootwait rw");
            }
            break; 
        case BOOT_SATA:
            printf("BOOT_SATA:\n");
